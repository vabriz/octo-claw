...
const plane = new THREE.Plane(new THREE.Vector3(0, 0, 2));
...
window.addEventListener('mousemove', (event) => {
    ...
    if (raycaster.ray.intersectPlane(plane, intersectionPoint)) {
      ...
      if (pivot) {
        // Convert world position to mesh local coordinates for bone movement
        // const localTargetPos = pivot.worldToLocal(intersectionPoint.clone());
        const localTargetPos = intersectionPoint.clone();
        pivot.z = 0; // constrain Z if needed
  
        pivot.position.copy(localTargetPos);
      }
    }
  });
